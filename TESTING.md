# PlexonCrates 2.0.0 verification
...
- weighted-selection boundaries, permission filtering, displayed chances, limits, cooldowns, and deterministic pity reset/guarantee behavior;
...
1. Start a clean server and confirm startup reports four crates, 32 rewards, schema 2, and live PlexonKeys templates.
...
7. Add item, command, XP, level, and money actions; edit rarity/weight/permissions/limits/messages/effects; test-deliver it; and verify test delivery changes no keys, statistics, limits, or pity.
...
17. Run `/pcrates validate`, `reload`, `backup`, and `diagnose`; then introduce an invalid weight, missing key, and duplicate exact key template and confirm activation is rejected without changing the live snapshot.
